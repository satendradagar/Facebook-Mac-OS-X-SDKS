...
// #endif
  ...
  NSViewController *_safariViewController;
...
- (instancetype)_init
{
  if ((self = [super init]) != nil) {
    ...
    [defaultCenter addObserver:self selector:@selector(applicationDidEnterBackground:) name:NSApplicationDidFinishLaunchingNotification object:nil];
...
//      [[NSApplication sharedApplication] openURL:url options:@{} completionHandler:^(BOOL success) {
//        handler(success, nil);
//      }];
//    } else {
//      BOOL opened = [[NSApplication sharedApplication] openURL:url];
//
...
//      }
//    }
    handler(false,[NSError errorWithDomain:@"Unsupported routine" code:100 userInfo:nil]);
  });
}
...
//    NSViewController *parent = fromViewController ?: [FBSDKInternalUtility topMostViewController];
//    if (parent == nil) {
...
- (void)safariViewControllerDidFinish:(NSViewController *)safariViewController
{
  if (_pendingURLOpen) {
    id<FBSDKURLOpening> pendingURLOpen = _pendingURLOpen;
