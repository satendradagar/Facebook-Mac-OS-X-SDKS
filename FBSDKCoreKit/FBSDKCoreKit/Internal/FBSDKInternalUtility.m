...
//  components.path = @"/";
//  return [[NSApplication sharedApplication] canOpenURL:components.URL];
}
...
+ (NSWindow *)findWindow
{
  ...
  if (window == nil || window.level != NSNormalWindowLevel) {
...
{
  NSWindow *keyWindow = [self findWindow];
  ...
  NSViewController *topController = keyWindow.windowController.contentViewController;
  while (topController.presentedViewControllers) {
    topController = topController.presentedViewControllers.firstObject;
  }
