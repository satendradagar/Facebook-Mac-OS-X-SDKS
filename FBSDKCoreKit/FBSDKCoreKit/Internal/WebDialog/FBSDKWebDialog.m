...
- (BOOL)show
{
  ...
  NSWindow *cWindow = [FBSDKInternalUtility findWindow];
  if (!cWindow) {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  _dialogView = [[FBSDKWebDialogView alloc] initWithFrame:cWindow.frame];
...
- (void)_dismissAnimated:(BOOL)animated
{
  ...
  NSView *backgroundView = _backgroundView;
  _backgroundView = nil;
...
//  };
//  if (animated) {
//    [NSView animateWithDuration:FBSDK_WEB_DIALOG_DISMISS_ANIMATION_DURATION animations:^{
//      dialogView.alphaValue = 0.0;
...
- (BOOL)_showWebView
{
  NSWindow *window = [FBSDKInternalUtility findWindow];
  if (!window) {
    [FBSDKLogger singleShotLogEntry:FBSDKLoggingBehaviorDeveloperErrors
                       formatString:@"There are no valid ViewController to present FBSDKWebDialog", nil];
    [self _failWithError:nil];
  ...
  _backgroundView = [[NSView alloc] initWithFrame:window.frame];
  _backgroundView.alphaValue = 0.0;
  _backgroundView.autoresizingMask = (kCALayerWidthSizable | kCALayerHeightSizable);
//  _backgroundView.backgroundColor = [UIColor colorWithWhite:0.3 alpha:0.8];
  [window.contentView addSubview:_backgroundView];

  [window.contentView addSubview:_dialogView];
...
//    return applicationFrame;
//  }
  return _dialogView.window.frame;
}
